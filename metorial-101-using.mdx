...
Now that you've deployed your first provider and confirmed it's working, you can connect it to an LLM provider like OpenAI.

In this guide, you'll learn how to build a chat-enabled app that automatically handles tool calls from your Metorial provider.
...
<Steps>
  ...
  <Step title="3. Fetch Your Provider Tools">
    Create a session that exposes your deployed provider tools. Steps 4–6 happen inside the session callback (TypeScript) or context manager (Python).
    ...
    await metorial.withProviderSession(
      ...
      async session => {
          // Steps 4–6 take place here, using session.tools and session.callTools()
      }
    );
    ```
