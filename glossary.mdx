...
This glossary summarizes core Metorial concepts. For deeper guides, see [Projects and Organizations](/concepts-projects-organizations), [Providers](/concepts-providers), and [Sessions](/concepts-sessions).
...
A magic MCP server is a resource type that can be assigned to [consumers](#consumer) alongside [providers](#provider), depending on your setup.
...
A provider deployment is the runtime instance that combines a [provider](#provider) with a [provider config](#provider-config) and [provider auth config](#provider-auth-config) and executes requests. Multiple deployments can share the same configs and scale independently.
