...
Skills can reach users in two ways:

| Mode | Use it when | How it works |
| --- | --- | --- |
| Admin-managed skills | An admin wants to publish approved skills, templates, or groups to a portal | Add a skill resource to the portal from the dashboard, choose the skill, template, or group, and allow the right access groups |
| User-created skills | A user wants to capture a workflow from the portal | Create a personal skill in the portal editor, keep it private while drafting, invite teammates to collaborate, or share it with groups that should have access |

Personal skills are useful when the person closest to the work knows the workflow best. A support lead can draft a refund handling skill, a sales teammate can write an account research skill, and an operations teammate can turn a recurring checklist into something agents can follow. Shared skills can be collaborative, so several teammates can refine the same instructions before the skill is used more broadly.

![Collaborative skill editor](/images/dashboard-audit/skill-collaborative-editor.png)

For admin-managed skills, the dashboard controls what appears in the portal and who can use it.
